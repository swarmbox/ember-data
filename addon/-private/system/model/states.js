...
// Map for state.dirtyType to SwarmBox CommitOperation
const SWARMBOX_OPERATION = {
  created: 'createRecord',
  updated: 'updateRecord',
  deleted: 'deleteRecord'
}

/* SwarmBox Transaction Logic */
function addToTransaction(internalModel, dirtyType) {
  const operation = SWARMBOX_OPERATION[dirtyType];
  const transaction = internalModel.store.transaction;
  if (operation && transaction) {
    transaction.add(internalModel, operation);
  }
}

function removeFromTransaction(internalModel, dirtyType) {
  const operation = SWARMBOX_OPERATION[dirtyType];
  const transaction = internalModel.store.transaction;
  if (operation && transaction) {
    transaction.remove(internalModel, operation);
  }
}
...
const DirtyState = {
  ...
  uncommitted: {
    ...
    pushedData(internalModel) {
      if (!internalModel.hasChangedAttributes()) {
        ...
        removeFromTransaction(internalModel, this.dirtyType);
      }
    },
  ...
  inFlight: {
    ...
    becameError(internalModel) {
      ...
      // SwarmBox: no changes to Transaction as it remains in the Transaction under correct Operation
    },
  },
  ...
  invalid: {
    ...
    deleteRecord(internalModel) {
      ...
      removeFromTransaction(internalModel, this.dirtyType);
      addToTransaction(internalModel, 'deleted');
    },
    ...
    becameValid(internalModel) {
      internalModel.transitionTo('uncommitted');
      // SwarmBox: no changes to Transaction as it remains in the Transaction under correct Operation (invalid -> valid)
    },
...
createdState.uncommitted.pushedData = function(internalModel) {
  ...
  // SwarmBox: think this can only happen if record from server has same id as this record, so its no longer created but updated
  removeFromTransaction(internalModel, 'created');
  addToTransaction(internalModel, 'updated');
};
...
updatedState.uncommitted.deleteRecord = function(internalModel) {
  ...
  removeFromTransaction(internalModel, 'updated');
  addToTransaction(internalModel, 'deleted');
};
...
const RootState = {
  ...
  empty: {
    ...
    loadedData(internalModel) {
      ...
      internalModel.triggerLater('ready');
      addToTransaction(internalModel, 'created');
    },
